...
inline arma::vec simpa_step(arma::vec current_x, 
  ...
  adaptparams.step();
  // diminishing adaptation: probability of updating preconditioner 
  ...
  arma::vec proposal_x = proposal_mean + eps1 * Minvchol.t() * p;
  ...
  if(adapt_now) {
    ...
    RR = model.compute_dens_grad_neghess(joint1, revgrad, proposal_x, true);
    ...
    RR = model.compute_dens_grad_neghess(joint1, revgrad, proposal_x, false);
  ...
  arma::vec reverse_mean = proposal_x + eps2 * 0.5 * Rinvchol.t() * Rinvchol * revgrad; 
  
  double prop0to1 = Micholdet -.5/eps2 * arma::conv_to<double>::from(
    (proposal_x - proposal_mean).t() * MM * (proposal_x - proposal_mean) );
